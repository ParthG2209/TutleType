...
class MultiplayerEngine {
    ...
    joinRoom(roomId) {
        ...
        console.log('Checking if room exists in Firebase...');

        if (db) {
            db.ref('rooms/' + roomId).once('value').then(snapshot => {
                console.log('📍 Room lookup snapshot:', snapshot.val());

                if (snapshot.exists()) {
                    console.log('✓ Room found! Adding player...');
                    ...
                    console.error('❌ Room does not exist. Room data:', snapshot.val());
